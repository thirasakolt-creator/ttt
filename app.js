...
//  Version: 1.2  แก้ไข CORS: เปลี่ยน POST → GET ทั้งหมด
// ============================================================
...
async function loadUserList() {
  ...
  if (!users) {
    try {
      ...
      grid.innerHTML = `<div class="user-loading" style="grid-column:1/-1;color:var(--red)">⚠️ โหลดไม่ได้ ลองใหม่</div>`;
      return;
    }
  }
...
async function checkRoutineThenLoad() {
  ...
  try {
    const res = await api({ action: "getRoutineLog", shift, date: today });
    if (res.done) {
      document.getElementById("routine-done-banner").classList.remove("hidden");
      document.getElementById("routine-done-text").textContent =
        `✅ ทำแล้วโดย ${res.done_by}` + (res.timestamp ? ` เวลา ${String(res.timestamp).split(" ")[1] || ""}` : "");
...
document.getElementById("btn-submit-routine").addEventListener("click", async () => {
  ...
  try {
    await api({ action: "saveRoutineLog", shift, date: today, done_by: user.name });

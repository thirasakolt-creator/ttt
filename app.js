...
async function openRepairModal(item){
  ...
  if(item.img_before){
    ...
    imgBefore.src=""; imgBefore.alt="กำลังโหลดรูป...";
    getDriveUrl(item.img_before).then(url=>{ if(url) imgBefore.src=url; });
  } else { imgBefore.style.display="none"; }

  // รูปหลังซ่อม
  const imgAfter=document.getElementById("modal-img-after");
  if(item.img_after){
    imgAfter.style.display="block"; imgAfter.src="";
    getDriveUrl(item.img_after).then(url=>{ if(url) imgAfter.src=url; });
  } else { imgAfter.style.display="none"; }

...
async function flushQueue(){
  // flush ทั้ง routine queue และ upload queue พร้อมกัน
  ...
  const doneCount=done.value||0;
  if(doneCount>0) showToast(`✅ sync ${doneCount} รายการสำเร็จ`,"success");
}
...
async function uploadPhotoAPI(b64, docNo, colLabel){
  // ★ ใช้ Content-Type: text/plain เพื่อหลีกเลี่ยง CORS preflight (OPTIONS)
  // Apps Script ไม่รองรับ preflight → ทำให้ upload ล้มเหลวเงียบๆ
  // text/plain ไม่ trigger preflight แต่ Apps Script ยังอ่าน postData.contents ได้ปกติ
  const payload=JSON.stringify({
    action:"uploadPhoto",
    doc_no: docNo||"unknown",
    col_label: colLabel||"รูปภาพ (หลังซ่อม)",
    data: b64
  });
...
function getTodayStr(){
  // คืนวันที่ ค.ศ. สำหรับ API query / cache key
  const d=new Date();
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
}
function getTodayThai(){
  // คืนวันที่ พ.ศ. รูปแบบ dd/mm/yyyy สำหรับบันทึกลง Sheet
  const d=new Date();
...
function enterApp(shift,fromSession){
  ...
  // ซ่อนปุ่มบ่อน้ำ
  const waterBtn=document.querySelector('.nav-btn[data-nav="water"]');
  ...
  // background sync ทันทีตอนเปิดแอป (ไม่รอ หน้า repairs)
  flushQueue().catch(()=>{});
  backgroundSyncAll().catch(()=>{});
  setupAutoLogout(shift);
  navigateTo("repairs");
}

// ── Background Sync ทุกระบบ ──
async function backgroundSyncAll(){
  setSyncStatus("syncing");
  try{
    // sync งานซ่อม
    const repRes=await api({action:"getRepairs"});
    if(repRes.success){
      state.repairs=repRes.data;
      store(K.REPAIRS,repRes.data);
      // pre-cache รูปทุกใบเบื้องหลัง
      repRes.data.forEach(r=>{
        if(r.img_before){
          const url=getDriveUrl(r.img_before);
          if(url) loadAndCacheImg(url,r.img_before).catch(()=>{});
        }
      });
      // อัปเดต UI ถ้ากำลังแสดงหน้างานซ่อมอยู่
      const repPanel=document.getElementById("screen-repairs");
      if(repPanel&&!repPanel.classList.contains("hidden")) renderRepairs();
    }
    // sync VIP (ถ้าเป็น VIP)
    if(isVIP(state.currentUser?.role)){
      const vipRes=await api({action:"getRepairsVIP"});
      if(vipRes.success){
        const curYear=getCurrentYear();
        const fy=arr=>arr.filter(r=>{const d=normalizeYear(r.date);return d&&parseInt(d.split("-")[0])===curYear;});
        const out=fy(vipRes.outsource), pen=fy(vipRes.pending_score);
        store(K_VIP_OUTSOURCE,out); store(K_VIP_PENDING,pen);
        updateVIPBadges(out.length,pen.length);
      }
    }
...
function buildRoutineCard(title,items){
  const rows=items.map(it=>`
    <div class="routine-input-row">
      <label>${it.description}</label>
      <div class="custom-num-wrap" id="wrap_${it.item_id}">
        <input class="routine-input" type="text" readonly
          id="ri_${it.item_id}" placeholder="0"
          data-value="0"
          onclick="openNumpad('${it.item_id}','${it.unit||''}')"/>
        <span class="routine-input-display" id="disp_${it.item_id}">0</span>
      </div>
      <span class="routine-unit">${it.unit||""}</span>
    </div>`).join("");
...
// ── Custom Numpad Modal ──
let _numpadTarget=null; // item_id ที่กำลังแก้

function openNumpad(itemId, unit){
  _numpadTarget=itemId;
  const cur=document.getElementById("ri_"+itemId)?.dataset.value||"0";
  document.getElementById("numpad-display").textContent=cur==="0"?"":cur;
  document.getElementById("numpad-unit").textContent=unit||"";
  document.getElementById("modal-numpad").classList.remove("hidden");
}

function closeNumpad(){
  document.getElementById("modal-numpad").classList.add("hidden");
  _numpadTarget=null;
}
...
function numpadTap(val){
  ...
  if(val==="⌫"){ cur=cur.slice(0,-1)||""; }
  else if(val==="."){ if(!cur.includes(".")) cur+="."; }
  else if(val==="C"){ cur=""; }
  else{ if(cur==="0"||cur==="") cur=val; else cur+=val; }
...
function numpadConfirm(){
  ...
  const disp=document.getElementById("numpad-display");
  const val=disp.textContent||"0";
  const inp=document.getElementById("ri_"+_numpadTarget);
  if(inp){ inp.value=val; inp.dataset.value=val; }
  const dispEl=document.getElementById("disp_"+_numpadTarget);
...
document.getElementById("btn-submit-routine").addEventListener("click",()=>{
  ...
  document.querySelectorAll(".routine-input").forEach(inp=>{
    // รองรับทั้ง custom numpad (dataset.value) และ text input ปกติ
    const val=inp.dataset.value||inp.value||"0";
    waterData[inp.id.replace("ri_","")]=val;
  });
  ...
  const doUpload=async()=>{
    ...
    if(shift==="morning"){
      ...
      // upload รูปก่อน
      const photoUrls=[];
      for(let pi=0;pi<state.afternoonPhotos.length;pi++){
        const photo=state.afternoonPhotos[pi];
        try{
          const up=await uploadPhotoAPI(
            photo.b64,
            `afternoon_${today}`,
            `รูปกะเที่ยง_${pi+1}`
          );
          if(up.success){
            photoUrls.push(up.path||up.url||"");
          }
        }catch(e){ Logger?.log?.("afternoon upload error: "+e); }
      }
      const ap={date:today,
...
function renderRepairs(){
  ...
  if(state.filter==="รอจัดซื้อ") items=items.filter(r=>r.status==="จะซื้ออุปกรณ์"||r.status==="รอจัดซื้ออุปกรณ์");
  else if(state.filter!=="all") items=items.filter(r=>r.status===state.filter);
  ...
  list.innerHTML=items.map(r=>{
    ...
    // lazy load รูปด้วย data-src แล้ว loadImg() จะโหลดจริงๆ
    const imgHtml=imgUrl?`<div class="rc-img-wrap"><img class="rc-thumb" data-src="${imgUrl}" data-path="${r.img_before||''}" src="" alt="" loading="lazy"/></div>`:"";
    return `<div class="repair-card status-${cls}" data-docno="${r.doc_no}">
      ...
      <div class="rc-meta">
        ...
        <span class="rc-tag">${toThaiDate(normalizeYear(r.date))||""}</span>
  ...
  // โหลดรูปแบบ lazy + retry
  lazyLoadRepairImages();
}

// ── Lazy Load + Retry Images ──
function lazyLoadRepairImages(){
  document.querySelectorAll(".rc-thumb[data-src]").forEach(img=>{
    ...
    img.onerror=()=>{
      // retry หลัง 3 วิ
      setTimeout(()=>{
        if(img.dataset.src) img.src=img.dataset.src+"&t="+Date.now();
      },3000);
    };
  });
}
...
function openRepairModal(item){
  ...
  document.getElementById("modal-date").textContent=toThaiDate(normalizeYear(item.date))||"-";
...
function updateTechRequired(status){
  ...
  techGroup.querySelector("label").innerHTML=req?"ผู้ซ่อมหลัก <span style='color:var(--red)'>*</span>":"ผู้ซ่อมหลัก";
  // บังคับชื่อช่างเฉพาะตอนปิดงาน
  const techInput=document.getElementById("inp-tech-main");
  techInput.required=req;
  // แสดง/ซ่อน after photo และ assistants เฉพาะปิดงาน
  ...
  // ซ่อนชื่อช่างสำหรับ รับทราบ
  techGroup.style.display=(status===""||status==="รับทราบ")?"none":"block";
}
...
document.getElementById("btn-update-status").addEventListener("click",async()=>{
  ...
  const params={row_index:state.selectedRepair.row_index,status:state.selectedStatus,
    ...
    date_done:state.selectedStatus==="ดำเนินการเรียบร้อย"?getTodayThai():"",img_after:""};
  ...
  // บันทึกลง queue ทันที (ทำงานแม้ปิดแอป)
  const uploadItem={type:"status",params:{...params}};

  const doUpload=async()=>{
    showUploadProgress("กำลัง sync...", 0);

    // upload รูปก่อน (ถ้ามี)
    if(state.afterPhotoB64){
      showUploadProgress("กำลังอัปโหลดรูป...", 30);
      try{
        const up=await uploadPhotoAPI(
          state.afterPhotoB64,
          state.selectedRepair.doc_no,
          "รูปภาพ (หลังซ่อม)"
        );
        if(up.success){
          showUploadProgress("อัปโหลดรูปสำเร็จ ✅", 60);
          params.img_after_path=encodeURIComponent(up.path||"");
          uploadItem.params.img_after_path=params.img_after_path;
          showToast(`✅ อัปโหลดรูป: ${up.filename||""}`, "success");
        }else{
          showToast("⚠️ อัปโหลดรูปไม่สำเร็จ จะลองใหม่", "");
        }
      }catch(e){
        showToast("⚠️ อัปโหลดรูปไม่ได้ จะลองใหม่ภายหลัง","");
      }
    }

    // อัปเดตสถานะ
    showUploadProgress("กำลังบันทึกสถานะ...", 80);
    try{
      const r=await api({action:"updateRepairFull",...uploadItem.params});
      if(r.success){
        showUploadProgress("บันทึกสำเร็จ ✅",100);
        setTimeout(hideUploadProgress,1500);
      }else{
        // เก็บใน queue รอ retry
        enqueueUpload(uploadItem);
        showUploadProgress("บันทึกไม่ได้ — รอ sync",100);
        setTimeout(hideUploadProgress,2000);
      }
    }catch(e){
      enqueueUpload(uploadItem);
      showUploadProgress("offline — รอ sync",100);
      setTimeout(hideUploadProgress,2000);
    }
  };
  doUpload().catch(e=>{
...
// ============================================================
// ── VIP cache keys ──
const K_VIP_OUTSOURCE="mcr_vip_outsource";
const K_VIP_PENDING  ="mcr_vip_pending";

async function loadVIPData(){
  // แสดง cache ก่อนเลย
  const cachedOut=load(K_VIP_OUTSOURCE,[]);
  const cachedPen=load(K_VIP_PENDING,[]);
  if(cachedOut.length||cachedPen.length){
    renderOutsourceList(cachedOut);
    renderPendingList(cachedPen);
    updateVIPBadges(cachedOut.length,cachedPen.length);
  }
  // sync เบื้องหลัง
  try{
    ...
    if(res.success){
      ...
      store(K_VIP_OUTSOURCE,outsource); store(K_VIP_PENDING,pending);
      renderOutsourceList(outsource);
      renderPendingList(pending);
      updateVIPBadges(outsource.length,pending.length);
    }
  }catch(e){if(!cachedOut.length) showToast("โหลดข้อมูล VIP ไม่ได้","error");}
  loadUserMgmt();
}

function updateVIPBadges(outCount,penCount){
  document.getElementById("badge-outsource").textContent=outCount;
  document.getElementById("badge-pending").textContent=penCount;
}

// ── Outsource list — อัปเดตสถานะได้ ──
function renderOutsourceList(items){
  const list=document.getElementById("list-outsource");
  if(!items.length){list.innerHTML=`<div class="empty-state">ไม่มีงานติดต่อช่างนอก</div>`;return;}
  list.innerHTML=items.map(r=>`
    <div class="repair-card" data-docno="${r.doc_no}" data-rowindex="${r.row_index}">
      <div class="rc-top">
      ...
      <div class="vip-action-row">
        <button class="vip-action-btn vip-btn-ok"   data-row="${r.row_index}" data-status="ดำเนินการเรียบร้อย">✅ เรียบร้อย</button>
        <button class="vip-action-btn vip-btn-ack"  data-row="${r.row_index}" data-status="รับทราบ">📋 รับทราบ</button>
        <button class="vip-action-btn vip-btn-cancel" data-row="${r.row_index}" data-status="ยกเลิกซ่อม">❌ ยกเลิก</button>
      </div>
    </div>`).join("");

  list.querySelectorAll(".vip-action-btn").forEach(btn=>{
    btn.addEventListener("click",async()=>{
      const rowIdx=btn.dataset.row, newStatus=btn.dataset.status;
      btn.disabled=true; btn.textContent="...";
      try{
        const res=await api({action:"updateRepairFull",row_index:rowIdx,status:newStatus,date_done:newStatus==="ดำเนินการเรียบร้อย"?getTodayThai():""});
        if(res.success){
          showToast(`✅ ${newStatus}`,"success");
          // ลบออกจาก cache และ render ใหม่
          const upd=load(K_VIP_OUTSOURCE,[]).filter(r=>String(r.row_index)!==String(rowIdx));
          store(K_VIP_OUTSOURCE,upd); renderOutsourceList(upd); updateVIPBadges(upd.length,load(K_VIP_PENDING,[]).length);
        }else showToast(res.message||"อัปเดตไม่ได้","error");
...
// ── Pending score list — ให้คะแนนได้ ──
function renderPendingList(items){
  const list=document.getElementById("list-pending");
  if(!items.length){list.innerHTML=`<div class="empty-state">ไม่มีงานรอให้คะแนน</div>`;return;}
  list.innerHTML=items.map(r=>`
    <div class="repair-card" data-rowindex="${r.row_index}">
      <div class="rc-top">
        <div class="rc-location">${r.location||"-"}</div>
      ...
      <div class="score-row">
        ...
        ${[1,2,3,4,5].map(n=>`<button class="score-star" data-row="${r.row_index}" data-score="${n}">⭐${n}</button>`).join("")}
      </div>
    </div>`).join("");

  list.querySelectorAll(".score-star").forEach(btn=>{
    btn.addEventListener("click",async()=>{
      const rowIdx=btn.dataset.row, score=btn.dataset.score;
      const inspector=state.currentUser?.name||"";
      btn.disabled=true; btn.textContent="...";
      try{
        const res=await api({action:"updateScore",row_index:rowIdx,score,inspector:encodeURIComponent(inspector)});
        if(res.success){
          showToast(`✅ ให้คะแนน ${score} แล้ว`,"success");
          const upd=load(K_VIP_PENDING,[]).filter(r=>String(r.row_index)!==String(rowIdx));
          store(K_VIP_PENDING,upd); renderPendingList(upd); updateVIPBadges(load(K_VIP_OUTSOURCE,[]).length,upd.length);
        }else showToast(res.message||"บันทึกไม่ได้","error");
      }catch(e){showToast("เกิดข้อผิดพลาด","error");}
    });
  });
}
...
function showUploadProgress(msg, pct){
  ...
  if(!bar){
    ...
    bar.style.cssText="position:fixed;bottom:70px;left:0;right:0;z-index:8000;background:var(--navy-2);border-top:1px solid var(--border);padding:10px 16px;font-size:.8rem;";
    document.body.appendChild(bar);
  }
  bar.innerHTML=`<div style="display:flex;justify-content:space-between;margin-bottom:6px"><span>${msg}</span><span>${pct}%</span></div>
    <div style="height:4px;background:var(--navy-4);border-radius:2px"><div style="height:100%;width:${pct}%;background:var(--gold);border-radius:2px;transition:width .3s"></div></div>`;
